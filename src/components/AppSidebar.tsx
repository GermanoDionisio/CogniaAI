...
export function AppSidebar({
  ...
  const { data: conversations = [] } = useQuery({
    queryKey: ["conversations", isAuthenticated ? "auth" : "guest"],
    queryFn: () => list(),
  ...
  function requireAuth(e: React.MouseEvent<HTMLAnchorElement | HTMLButtonElement>, mode: "login" | "signup" = "login") {
    if (!isAuthenticated) {
      e.preventDefault();
      onOpenAuth(mode);
      onClose();
            ...
            <SidebarSection label="Histórico" icon={MessageSquare}>
              <p className="text-xs text-muted-foreground px-3 py-2">
                {isAuthenticated
                  ...
                  : "As conversas do modo visitante ficam só nesta sessão."}
              </p>
            </SidebarSection>
          )}
          ...
          {isAuthenticated ? (
            ...
            <div className="grid grid-cols-2 gap-2 pt-1">
              <button
                ...
                className="rounded-lg border border-border/60 px-3 py-2 text-sm hover:bg-sidebar-accent transition-colors"
              >
                Entrar
              </button>
              <button
